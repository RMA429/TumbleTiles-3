...
class TileEditorGUI:
	def __init__(self, parent, tumbleGUI, board, glue_data, previewTileList):
		...
		optionsMenu.add_checkbutton(label="Show Location", onvalue=True, offvalue=False, variable=self.tkSHOWLOC, command = lambda: self.redrawPrev())
		...
		optionsMenu.add_command(label="Cloud Save", command = lambda: self.CloudSave())
		...
		for prevTile in self.prevTileList:
			...
			if prevTile.isConcrete == False:
				...
				if prevTile.glues[0] != "None":
					#north
					self.tilePrevCanvas.create_text(int(x + size/2), int(y + size/5), text = prevTile.glues[0], fill="#000", font=('', int(size/5)) )
				if prevTile.glues[1] != "None":
					#east
					self.tilePrevCanvas.create_text(int(x + size - size/5), int(y + size/2), text = prevTile.glues[1], fill="#000", font=('', int(size/5)))
				if prevTile.glues[2] != "None":
					#south
					self.tilePrevCanvas.create_text(int(x + size/2), int(y + size - size/5), text = prevTile.glues[2], fill="#000", font=('', int(size/5)) )
				if prevTile.glues[3] != "None":
					#west
					self.tilePrevCanvas.create_text(int(x + size/5), int(y + size/2), text = prevTile.glues[3], fill="#000", font=('',int(size/5)) )
